...
const slides = [
    ...
    },
    {
        ...
        texts: [
            ...
            { title: "NATURE", description: "A world that exists beyond the noise.", position: "posCenterLeft" },
            { title: "BREATHE", description: "Slow down and notice where you are.", position: "posTopRight" },
            { title: "FREE", description: "Nothing ahead but open space.", position: "posBottomLeft" }
        ]
    },
    {
        ...
        texts: [
            ...
            { title: "REMEMBER", description: "The places remain long after you leave.", position: "posCenterLeft" },
            { title: "AGAIN", description: "Every ending hides another beginning.", position: "posBottomRight" },
            { title: "START", description: "And the story begins again.", position: "posTopLeft" }
        ]
    }
];

export default function SponsorsCinematicScroll() {
    ...
    useEffect(() => {
        ...
        const getReveal = (type: string, progress: number) => {
            ...
            if (type === "bottom") {
                const y = 100 - p * 115;
                return `polygon(0% ${y}%, 12% ${y - 4}%, 25% ${y + 2}%, 40% ${y - 3}%, 55% ${y + 2}%, 70% ${y - 3}%, 85% ${y + 2}%, 100% ${y - 2}%, 100% 100%, 0% 100%)`;
            }
            if (type === "diagonal") {
                const x = -110 + p * 220;
                return `polygon(${x}% 0%, ${x + 35}% 0%, ${x + 135}% 100%, ${x + 100}% 100%)`;
            }
            if (type === "vertical") {
                const width = p * 115;
            ...
            if (type === "organic") {
                const radius = p * 160;
                return `circle(${radius}% at 47% 53%)`;
            }
            if (type === "right") {
                const x = 110 - p * 220;
                return `polygon(${x}% 0%, 100% 0%, 100% 100%, ${x - 35}% 100%)`;
            }
            if (type === "center") {
                const radius = p * 160;
                return `circle(${radius}% at 50% 50%)`;
            }
            return `circle(${p * 160}% at 50% 50%)`;
        };
